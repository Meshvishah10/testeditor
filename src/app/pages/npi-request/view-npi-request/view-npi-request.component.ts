...
@Component({
  selector: 'app-view-npi-request',
  templateUrl: './view-npi-request.component.html',
  styleUrl: './view-npi-request.component.scss'
})
export class ViewNpiRequestComponent implements OnInit , AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.ticketEditForm.get('Title').valueChanges.subscribe((value:any) => {
      ...
      if (value?.toLowerCase() === 'other') {
        this.ticketEditForm.get('Miscoption').setValidators([Validators.required]);
      } else {
        // If 'Title' is not 'other', remove the 'required' validator from 'other' control
        this.ticketEditForm.get('Miscoption').clearValidators();
      }
      // Update the validation status of 'other' control
      this.ticketEditForm.get('Miscoption').updateValueAndValidity();
    });
  ...
  getTicketDetail() {
    ...
    this.api
      .callApi(this.constant.detailNPIRequest +
        '?inputRequest=' +
  ...
  deleteTicket() {
    ...
    this.api
      .callApi(this.constant.commonNPIRequest +
        '?inputRequest=' +
  ...
  editModel() {
    this.api
      .callApi(
        this.constant.commonNPIRequest +
        '?inputRequest=' +
  ...
  submit() {
    ...
    if (this.ticketEditForm.invalid || this.customvalidation !== ''){  
      ...
      this.api
      .callApi(this.constant.commonNPIRequest,body, 'PUT', true, true)
      .subscribe(
        (res: any) => {
          if (res.Status == 1) {
            this.ticketEditForm.reset();
  ...
  addSubmit(){
   ...
   if (this.callLogAddForm.invalid){  
    ...
    this.api
    .callApi(this.constant.callLogNPIRequest,body, 'POST', true, true)
    .subscribe(
      (res: any) => {
        if (res.Status == 1) {
...
printDiv() {
  ...
  html2canvas(printContent, { scale: 3 }).then(canvas => {
    ...
    doc.save(`ticket_${this.ticket_number}.pdf`);
